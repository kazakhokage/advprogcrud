...
func init() {
	os.Setenv("DATABASE_URL", "user=postgres dbname=advprog password=esengali123 sslmode=disable")
	var err error
	...
	//crud
	db.Create(&User{Username: "esen", Email: "esen@gmail.com", Password: "esen"})
	// db.Model(&User{}).Where("id = ?", 2).Update("username", "aigali")
